 ...
 <form id="main_form" action="insert" method="post">
  <table class="form">
   <tbody>
    ...
    <tr>
     <th>${e:label("phone")}:</th><td><input type="tel" ${f:text("phone")} required="required" maxlength="13" size="13" class="${f:errorClass('phone', 'error')}"/>${e:error('phone')}</td>
    </tr>
    <tr>
     <th>${e:label("zipcode")}:</th><td><input type="text" ${f:text("zipcode")} required="required" maxlength="8" size="8" class="${f:errorClass('zipcode', 'error')}">${e:error('zipcode')}</td>
    </tr>
    <tr>
     <th>${e:label("address")}:</th><td><input type="text" ${f:text("address")} required="required" maxlength="256" size="50" class="${f:errorClass('address', 'error')}"/>${e:error('address')}</td>
    </tr>
   </tbody>
  </table>
