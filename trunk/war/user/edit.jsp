...
<div id="body_contents">
 ...
 <form id="main_form" action="update" method="post">
  ...
  <table class="form">
   <tbody>
    <tr>
     <th>${e:label("uid")}:</th><td><input type="text" ${f:text("uid")} readonly="readonly" size="16" class="${f:errorClass('uid', 'error')}"/>${e:error('uid')}</td>
    </tr>
    <tr>
    ...
    <tr>
     <th>${e:label("mail")}:</th><td><input type="email" ${e:text("mail")} required="required" maxlength="256" size="30" class="${f:errorClass('mail', 'error')}"/>${e:error('mail')}</td>
    </tr>
   </tbody>
  </table>
